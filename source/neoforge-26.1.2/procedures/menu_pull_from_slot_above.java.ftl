...
BlockPos pull_targetPos = BlockPos.containing(${input$X}, ${input$Y}, ${input$Z});

Direction pull_dir = Direction.${field$DIRECTION};

BlockPos pull_qmp_Pos = pull_targetPos.relative(pull_dir);

boolean pull_movedItem = false;

if (pull_lvl.getBlockEntity(pull_qmp_Pos) instanceof Container sourceContainer &&
    pull_lvl.getBlockEntity(pull_targetPos) instanceof Container targetContainer) {
